...
class CorporationsController < ApplicationController
  ...
  def edit
    @page_header = "Редактирование Корпорации"
    @corporation = Corporation.find(params[:id])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @corporation.save
        format.html { redirect_to corporations_path, notice: 'Corporation was successfully created.' }
        format.json { render json: @corporations, status: :created, location: @corporation }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @corporation.update_attributes(params[:corporation])
        format.html { redirect_to corporations_path, notice: 'Corporation was successfully updated.' }
        format.json { head :ok }
      else
